...
flowchart TD
    ...
    J --> K{"MFA enabled?"}
    K -->|No| L["Password-only account takeover"]
    K -->|Yes| M["Second factor required; bypass not measured"]
...
| Login after cracking | Known password becomes account access | MFA state | Password-only takeover count and second-factor challenge count |
...
- The dashboard models MFA as a binary second-factor gate, while real MFA strength depends on the factor type, recovery process, user behaviour, and phishing resistance.
- MFA is evaluated after offline cracking because it is not a password-quality property; it controls whether a known password is enough for password-only login in the scenario model.
